...
export async function middleware(req: NextRequest) {
  ...
  // if (!token?.backendToken.access_token) {
  ...
  //   const roleRedirect = redirectIfRoleMismatch(req, token.user.roleName, "/app/group");
  //   if (roleRedirect) return roleRedirect;
  // }

  // // ตรวจสอบ path สำหรับ /app/dashboard
  // if (urlPath.startsWith("/app/dashboard")) {
  //   const roleRedirect = redirectIfRoleMismatch(req, token.user.roleName, "/app/dashboard");
  //   if (roleRedirect) return roleRedirect;
  // }

  // // ตรวจสอบสิทธิ์การเข้าถึง role
  // const allowedRoles = ["student", "teacher", "admin"];
  // if (!allowedRoles.includes(token.user.roleName)) {
